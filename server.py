...
"""
Server script for running the DataNova API.
"""
import os
import asyncio
import argparse
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run the DataNova API server")
    parser.add_argument(
        "--reload",
        action="store_true",
    ...
    try:
        logger.info(f"Starting DataNova API server on {args.host}:{args.port}")
        uvicorn.run(
            "src.server:app",
            host=args.host,
            port=args.port,
